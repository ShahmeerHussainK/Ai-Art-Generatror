...
const DarkSketchToImagesUpload = () => {
  ...
  const prompt = "change this Sketch to image";
  const style_id = 29;
  const pickImage = () => {
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      const asImageSrc = await uploadImageGeneration(selectedImage, prompt, "29", "remix", "20", "lineart", "50", "15");
      navigation.navigate('GeneratingArtwork', { asImageSrc, prompt, style_id, selectedImage });
      setLoading(false);
      ...
      >
        ...
        >
          <View style={styles.autoLayoutVertical1FlexBox}>
            ...
            <Text style={[styles.uploadYourPhoto1,{ color: theme.text }]}>{(t("Upload your sketch"), t("and we’ll generate an AI image version foryou."))}
            </Text>
          </View>
          {showImage && (
...
export default DarkSketchToImagesUpload;